  ...
  for (const [pkg, expectedVersion] of Object.entries(criticalDependencies)) {
    ...
    if (!actualVersion) {
      ...
      if (cleanActual === cleanExpected) {
        ...
        console.log(
          `⚠️  ${pkg}: Expected ${expectedVersion}, found ${actualVersion}`,
        )
        allCorrect = false
      }
    }
...
function verifyNodeModules() {
  ...
  console.log(
    `✅ ${installedCount}/${Object.keys(criticalDependencies).length} critical packages found`,
  )
...
function verifyCompatibility() {
  ...
  try {
    ...
    if (reactMajor === 19 && nextMajor === 15) {
      ...
      console.log(
        `⚠️  Version mismatch: React ${reactMajor}, Next.js ${nextMajor}`,
      )
      return false
    }
  } catch (error) {
...
function generateReport() {
  ...
  console.log(
    `📦 Package Versions: ${report.packageJsonVersions ? '✅ PASS' : '❌ FAIL'}`,
  )
  console.log(
    `📁 Node Modules: ${report.nodeModulesInstallation ? '✅ PASS' : '❌ FAIL'}`,
  )
  console.log(`🔗 Import Tests: ${report.importTests ? '✅ PASS' : '❌ FAIL'}`)
  console.log(
    `🔄 Compatibility: ${report.compatibilityCheck ? '✅ PASS' : '❌ FAIL'}`,
  )
  ...
  if (allPassed) {
    console.log(
      '🎉 ALL CHECKS PASSED! Dependencies are updated and compatible.',
    )
    console.log('\n📋 Next Steps:')
